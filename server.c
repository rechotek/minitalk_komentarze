...
/*   Updated: 2024/05/17 16:37:55 by mrechuli         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// Czyli najpierw wpada 0, wiec wywolywana jest funkcja sig_2. Zwieksza ona g_state.index o 1, ale g_state.character pozostaje bez zmian (ustawlem go na poczatku na {0, 0}, wiec jak chce miec zero to nie robie nic tylko przechodze o jedno miejsce do przodu w indexie)
...
// Jak go wypisze to ustawiam z powrotem indeks i g_state.character na 0

// static oznacza ze zmienna g_state jest dostepna tylko w pliku w ktorym zostala zdefiniowna
// jest to funkcja globalna, poniewaz jest zdefiniowana poza funkcja, ale przez static jest dostepna tylko w tym pliku
static t_state g_state = {0,0}; // inicjalizauje globalna zmienna g_state, ktora ustawia wartosci pol struktury t_state na 0. W tym przypadku ustawia wartosc character oraz index na zero. Globalna zmienna musi sie zaczynac od "g_". Program dziala jak sie inaczej nazwie, ale norminette tego wymaga.
...
void	sig_1(int sig) // ta funkcja jest wywolywana gdy program otrzyma sygnal SIGUSR1
{
	...
	g_state.character |= (1 << g_state.index); // | jest to operator OR, jego dzialanie wytlumaczylem w WORDie. W tym kontekscie zamieni dane miejsce w indexie na 1 (bo w OR 0 i 1, 1 i 0, 1 i 1 dadza 1). W zadaniu ustawilem globalna zmienna na {0, 0}, wiec kazde wywolanie tego zmieni dane indexowe miejsce na 1 
	g_state.index++; 
	if (g_state.index == 8)
	{
